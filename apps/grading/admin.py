...
class GradingTaskAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('submission', 'submission_answer', 'grading_method', 'status')
        }),
        (_('Task Execution'), {
...
class GradingFeedbackAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('submission_answer',)
        }),
        (_('Feedback'), {
            'fields': ('feedback_text', 'score_breakdown')
        }),
        (_('Analysis'), {
            'fields': ('strengths', 'improvements', 'grading_confidence')
        }),
        (_('Grader'), {
            'fields': ('graded_by',)
        }),
...
class GradingConfigurationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('scope', 'exam', 'question', 'is_active')
        }),
        (_('Service Configuration'), {
            'fields': ('grading_service', 'service_config')
        }),
        ...
        (_('LLM Prompts'), {
            'fields': ('system_prompt', 'grading_prompt_template'),
            'classes': ('collapse',)
        }),
    )
