...
class GradingTask(TimeStampedModel):
    ...
    result = models.JSONField(
        null=True,
        blank=True,
        help_text=_('Grading result data')
    )
    error_message = models.TextField(
        blank=True,
        help_text=_('Error message if grading failed')
    )
...
class GradingFeedback(TimeStampedModel):
    ...
    feedback_text = models.TextField(
        help_text=_('Detailed feedback from grading')
    )
    score_breakdown = models.JSONField(
        null=True,
        blank=True,
        help_text=_('Detailed score breakdown')
    )
    strengths = models.JSONField(
        null=True,
        blank=True,
        help_text=_('List of strengths identified in the answer')
    )
    improvements = models.JSONField(
        null=True,
        blank=True,
        help_text=_('List of areas for improvement')
    )
...
class GradingConfiguration(TimeStampedModel):
    ...
    service_config = models.JSONField(
        default=dict,
        help_text=_('Service-specific configuration (model, temperature, etc.)')
    )
    ...
    require_manual_review = models.BooleanField(
        default=False,
        help_text=_('Always require manual review regardless of confidence')
    )

    # Timeout and retry
    grading_timeout = models.PositiveIntegerField(
        default=300,
        help_text=_('Grading timeout in seconds')
    )
    max_retries = models.PositiveIntegerField(
        default=3,
        help_text=_('Maximum retry attempts for failed grading')
    )

    # Prompt templates (for LLM grading)
    system_prompt = models.TextField(
        blank=True,
        help_text=_('System prompt template for LLM grading')
    )
    grading_prompt_template = models.TextField(
        blank=True,
        help_text=_('Grading prompt template with placeholders')
    )
    ...
    def clean(self):
        ...
        if self.scope == 'question' and not self.question:
            raise ValidationError({'question': _('Question is required for question-specific configuration')})
